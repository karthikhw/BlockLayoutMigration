...
public class BlockLayoutMigration {
        ...
        public void run() {
            while (!blockQueue.isEmpty()) {
                ...
                if (source != null){
                    ...
                    try{
                        long sleep = 60000;
                        int retry = 5, attempt = 1;
                        while(blockQueue.isEmpty() && retry != 0){
                            System.out.println(Thread.currentThread().getName()+" Sleeping.."+sleep+" ms Queue is empty now, no blocks to move. maxAttempt=5 retryAttempt="+attempt++);
                            Thread.sleep(sleep);
                            retry--;
                        }
                    }catch (Exception e){
    ...
    public static void populateQueue(File vol) {
        try {
            ...
            System.out.println("Total no of files including meta "+vol.toString() +" count="+files.length);
            for (File file : files) {
                if (file.isDirectory()) {
                    populateQueue(file);
                } else {
